...
// UPDATE
// update tutor date terminated
void updateDateTerminated(string newDateTerminated, int position)
{
	Tutor* current = head;
	int count = 1;
	...
	while (current != NULL)
	{
		if (count == position)
		{
			current->dateTerminated = newDateTerminated;
		}
...
void updateTutor(Tutor* head)
{
	...
	if (current->tutorID == searchTutorID)
	{
		...
		case 1:
			...
			updateDateTerminated(newDateTerminated, position);
			cout << "\"" << current->name << "\" date terminated updated successfully!" << endl << endl << endl;
